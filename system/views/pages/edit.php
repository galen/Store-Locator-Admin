...
<h2><?php if( $registry->controller == 'edit' ): ?>Editing Store #<?php e(  $registry->request->getParam( 'store_id' ) ) ?><?php else: ?>Create Store<?php endif; ?></h2>

<?php require( DIR_VIEWS . '/widgets/page_status_message.php' ) ?>

<form action="<?php if( $registry->controller == 'edit' ): ?><?php e(  URL_EDIT ) ?>/<?php e(  $registry->request->params->store_id ) ?>/<?php else: ?><?php e(  URL_CREATE ) ?>/<?php endif; ?>" method="post" id="store_edit">

	<fieldset>
		<div id="form_fields">
			<?php if( $registry->controller == 'edit' ): ?>
				<input type="hidden" name="<?php e(  $config['column_map']['id'] ) ?>" value="<?php e(  $store->getID() ) ?>" id="store_id">
			<?php endif; ?>
			<?php foreach( $registry->columns_edit as $property ): ?>
				...
				<?php endif; ?>
					<div>
						...
						<input type="text" name="<?php e(  $property ) ?>" id="<?php e(  $property ) ?>" value="<?php if ( isset( $store ) ): ?><?php e( $store->$property ) ?><?php endif; ?>">
					</div>
		...
		<div class="well" id="store_edit_actions">
			...
			<?php if( $registry->controller == 'edit' ): ?>
				<a href="<?php e(  URL_DELETE ) ?>/<?php e(  $store->getID() ) ?>/?c=<?php e(  URL_ROOT . '/' . REQUEST ) ?>/" id="delete_store" class="btn danger">Delete this store</a>
			<?php else: ?>
