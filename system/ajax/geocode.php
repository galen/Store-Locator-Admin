...
if ( $geocode instanceof \PHPGoogleMaps\Service\GeocodeResult ) {
	if ( isset( $_GET['save_store'] ) ) {
		...
		$store_save = new Store( $config['column_map'], array( $config['column_map']['id'] => $_GET[$config['column_map']['id']], $config['column_map']['lat'] => $geocode->getLat(), $config['column_map']['lng'] => $geocode->getLng() ) );
		$stg = new StoreTableGateway( $db, STORE_LOCATIONS_TABLE, $config['column_map'] );
